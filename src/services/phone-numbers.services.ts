...
export class PhoneNumbersServices {
    ...
    getPhoneNumbersList(requestDetails: any): Promise<any> {
        return this._req.get(`v1/sip/outers?page=${requestDetails.page}&limit=${requestDetails.limit}&search=${requestDetails.search && encodeURI(requestDetails.search)}`);
    }

    removePhoneNumber(id): Promise<any> {
        return this._req.del(`v1/sip/outers/${id}`);
    }

    toggleNumber(id, status): Promise<any> {
        return this._req.put(`v1/sip/outers/${id}/status`, {enable: status});
    }

    buyNumber(params): Promise<any> {
        // console.log(params);
        return this._req.post(`v1/number/fast_order`, params);
    }
    ...
    getAvailableNumbersList(requestDetails: any): Promise<any> {
        ...
        return this._req.post(`v1/number/catalog/find`, requestDetails);
    }
